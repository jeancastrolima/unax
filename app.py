        ...
        if empresa_escolhida and empresa_escolhida != "Selecione...":
            ...
            if laudo_selecionado:
                ...
                if dados_laudo and resultados_analise:
                    ...
                    # Garanta que coleta_id exista (exemplo: pegando de dados_laudo)
coleta_id = dados_laudo.get('id_coleta', 'default')

if st.button("Gerar e Guardar Diagnóstico de IA", type="primary", key=f"btn_gerar_{coleta_id}"):

    # 1. Placeholder do robô
    robot_placeholder = st.empty()

    # 2. Mostrar robô dançando
    robot_placeholder.markdown(
        ...
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; margin-top:20px;">
            <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExbDIzdGkxNmp2dnFwdTJ6M3ZyNXE1amh4ejhwMWg2MnRmbHpuYXV4diZlcD12MV9naWZzX3NlYXJjaCZjdD1n/uV6R7IyafWXtWkCkYW/giphy.gif" 
                 width="200">
            <p style="font-weight:bold; color:#007bff; margin-top:10px;">🤖 Alexandrinho está analisando...</p>
        </div>
        """,
        unsafe_allow_html=True
    )
    ...
    if "error" in analysis_result:
        ...
        with st.spinner("Guardando análise no banco de dados..."):
            save_status = backend.salvar_diagnostico_completo_ia(
                dados_laudo, 
                ai_response, 
                detailed_results
            )
        ...
        if nota_g == "Crítico":
            st.error(f"**Nota:** {nota_g}")
        elif nota_g == "Alerta":
            st.warning(f"**Nota:** {nota_g}")
        else:
            st.success(f"**Nota:** {nota_g}")

        # 7. Colunas de Diagnóstico (PT/EN)
        ...
        if "Status Calculado" in df_detalhado.columns:
            df_detalhado["Status Calculado"] = df_detalhado["Status Calculado"].apply(
                formatar_status_com_icone
            )
